...
class Decoder(nn.Module):
    ...
    def predict_beam(self, a, hidden, B ): # USING BEAM SEARCH
        ...
        for pos in range(l): # for each position in a given caption
            ...
            for b in range(B):
                hze, hidden = self.calstm.predict(a, hidden, prev_word=prev_words[b])
            ...
            """
            for i, (caption, word, should_add) in enumerate(zip(top_caps,top_words, add_more)):
                #print('cap (0):', len(caption), len(caption[0]),'words:', word,'SA:',should_add)
                
                if pos > 0:
                    for bbb in range(B):
                        if caption[bbb][-1]=='\\eos' and should_add[bbb]==True:
                            print('ERRORRRRRR', i , bbb)
                            raise NotImplemented
                            should_add[bbb]=False
                                
                if not any(should_add):
                    #print('cont after batch',i,'cap:',caption)
                    continue 
                for bb, (cap , w , sa) in enumerate(zip(caption,word,should_add)):
                    print('\n captions A:',id(cap),cap)
                    print('SA A', should_add)
                    if sa:
                        print('bb:', bb, self.vocab.idx2word[word[bb].item()])
                        cap.append(self.vocab.idx2word[word[bb].item()])
                        #caption[bb].append(self.vocab.idx2word[word[bb].item()]) # append the new words to the captions
                    if w == self.vocab.word2idx['\eos']:
                        should_add[bb] = False
                    print('captions B:',cap)
                    print('SA  B', sa)
                    #if not any(should_add):
                    #    break
