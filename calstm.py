...
class AttentionBlock(nn.Module):
    def __init__(self, L, D, hidden_size):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(L * (D + hidden_size), 256, bias=False),
            nn.Tanh(),
            nn.Linear(256, 128, bias=False),
            nn.Tanh(),
            nn.Linear(128, L, bias=False),
            nn.Softmax(dim=1)
        )
...
class CALSTM(nn.Module):
    ...
    def forward(self, a, hidden, y):
        padding = torch.tensor([0] * y.size(0), device=y.get_device()).view(-1, 1)
...
class ImageToLatex(nn.Module):
    def __init__(self, L, D, hidden_size, embedding_size, n_layers, vocab):
        ...
        self.decoder = Decoder(L, D, hidden_size, embedding_size, n_layers, vocab)
